...
//  SchoolSecurity
//
//  Created by zhangming on 17/10/13.
...
+ (instancetype)sharedInstance{
    ...
    static MyCoreDataManager *sharedClient = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedClient = [[MyCoreDataManager alloc] init];
    });
    return sharedClient;
}
...
- (instancetype)createCoredataDB:(NSString * )DBname
{
    ...
    if( !error ){
        //创建对象管理上下文，并设置数据解析器
        [MyCoreDataManager sharedInstance].context = [[NSManagedObjectContext alloc] initWithConcurrencyType:NSPrivateQueueConcurrencyType];
        [MyCoreDataManager sharedInstance].context.persistentStoreCoordinator = storeCoordinator;
        NSLog(@"数据库打开成功！");
    }else{
...
{
    NSEntityDescription * classDescription = [NSEntityDescription entityForName:NSStringFromClass(modelclass) inManagedObjectContext:[MyCoreDataManager sharedInstance].context];
    id modelobject = [[modelclass alloc] initWithEntity:classDescription insertIntoManagedObjectContext:[MyCoreDataManager sharedInstance].context];
    Coredatamodel(modelobject);
    NSError * error = nil;
    [[MyCoreDataManager sharedInstance].context save:&error];
...
{
    NSEntityDescription *description = [NSEntityDescription entityForName:NSStringFromClass(modelclass) inManagedObjectContext:[MyCoreDataManager sharedInstance].context];
    ...
    NSError *error = nil;
    NSArray *datas = [[MyCoreDataManager sharedInstance].context executeFetchRequest:request error:&error];
    if (!error && datas && [datas count])
    {
        for (NSManagedObject *obj in datas)
        {
            [[MyCoreDataManager sharedInstance].context deleteObject:obj];
            
        }
        
        if (![[MyCoreDataManager sharedInstance].context save:&error])
        {
            isresult(NO);
        }
        else
