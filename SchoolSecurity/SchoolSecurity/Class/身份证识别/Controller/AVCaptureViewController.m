...
-(void)viewDidLoad {
    ...
    [self setupBackBtnNavBarWithTitle:@"扫描身份证"];
    ...
    if (CVPixelBufferLockBaseAddress(imageBuffer, 0) == kCVReturnSuccess) {
        ...
        if (ret <= 0) {
            ...
            while(i < ret){
                ...
                if(xlen) {
                    ...
                    if(ctype == 0x21) {
                        ...
                        NSString *dateStr = [NSString stringWithCString:(char *)content encoding:gbkEncoding];
                        
                        NSString *str = [dateStr substringToIndex:8];
                        
                        NSRange range = NSMakeRange(9, 8);
                        
                        NSString *str2 = [dateStr substringWithRange:range];
                        
                        NSRange yearRange = NSMakeRange(0, 4);
                        NSString *year = [str substringWithRange:yearRange];
                        NSRange monthRange = NSMakeRange(4, 2);
                        NSString *month = [str substringWithRange:monthRange];
                        NSRange dayRange = NSMakeRange(6, 2);
                        NSString *day = [str substringWithRange:dayRange];
                        
                        iDInfo.start_date = [NSString stringWithFormat:@"%@/%@/%@",year,month,day];
                        
                        yearRange = NSMakeRange(0, 4);
                        year = [str2 substringWithRange:yearRange];
                        monthRange = NSMakeRange(4, 2);
                        month = [str2 substringWithRange:monthRange];
                        dayRange = NSMakeRange(6, 2);
                        day = [str2 substringWithRange:dayRange];
                        iDInfo.end_date = [NSString stringWithFormat:@"%@/%@/%@",year,month,day];
                    }
                }
            }
            ...
            if (iDInfo) {// 读取到身份证信息，实例化出IDInfo对象后，截取身份证的有效区域，获取到图像
                ...
                NSString *str = [iDInfo.num substringWithRange:range];
                NSRange yearRange = NSMakeRange(0, 4);
                NSString *year = [str substringWithRange:yearRange];
                NSRange monthRange = NSMakeRange(4, 2);
                NSString *month = [str substringWithRange:monthRange];
                NSRange dayRange = NSMakeRange(6, 2);
                NSString *day = [str substringWithRange:dayRange];
                iDInfo.birth = [NSString stringWithFormat:@"%@/%@/%@",year,month,day];
                
                
                NSLog(@"\n正面\n姓名：%@\n性别：%@\n民族：%@\n出生：%@\n住址：%@\n公民身份证号码：%@\n\n反面\n签发机关：%@\n签发日期：%@\n失效日期：%@",iDInfo.name,iDInfo.gender,iDInfo.nation,iDInfo.birth,iDInfo.address,iDInfo.num,iDInfo.issue,iDInfo.start_date,iDInfo.end_date);
                
                if (iDInfo.name.length > 0) {
                    
                    [IDInfoList sharedInstance].idInfo.name = iDInfo.name;
                    
                    [IDInfoList sharedInstance].idInfo.gender = iDInfo.gender;
                    
                    [IDInfoList sharedInstance].idInfo.nation = iDInfo.nation;
                    
                    [IDInfoList sharedInstance].idInfo.address = iDInfo.address;
                    
                    [IDInfoList sharedInstance].idInfo.num = iDInfo.num;
                    
                    [IDInfoList sharedInstance].idInfo.birth = iDInfo.birth;
                }
                if (iDInfo.issue.length > 0) {
                    
                    [IDInfoList sharedInstance].idInfo.issue = iDInfo.issue;
                    
                    [IDInfoList sharedInstance].idInfo.start_date = iDInfo.start_date;
                    
                    [IDInfoList sharedInstance].idInfo.end_date = iDInfo.end_date;
                }
                ...
                if (self.type == 3) {
                    ...
                    if ([IDInfoList sharedInstance].idInfo.num > 0) {
                        ...
                        [self viewWillAppear:year];
                    }
                }else{
                    
                    if ( [IDInfoList sharedInstance].idInfo.issue.length > 0 &&  [IDInfoList sharedInstance].idInfo.name.length > 0) {
                        
                        
                        dispatch_async(dispatch_get_main_queue(), ^{
                            
                            if (self.idInfoBlock != nil) {
                            ...
                            if (self.type == 1 ) {
                                ...
                                SMIdCardViewController *smidVC = [[SMIdCardViewController alloc] init];
                                smidVC.type = 1;
                                [self.navigationController pushViewController:smidVC animated:YES];
                        ...
                        dispatch_async(dispatch_get_main_queue(), ^{
                            ...
                            [MBProgressHUD showSuccess:@"继续读取另一面信息" toView:self.view];
